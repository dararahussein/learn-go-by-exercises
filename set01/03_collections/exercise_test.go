...
func testDoubled(t *testing.T) {
	if got := Doubled([]int{1, 2, 3}); !slices.Equal(got, []int{2, 4, 6}) {
		t.Fatalf("Doubled([1 2 3]): got %v, want [2 4 6]", got)
	}
}

func testArrayTotal(t *testing.T) {
	if got := ArrayTotal([3]int{10, 20, 30}); got != 60 {
		t.Fatalf("ArrayTotal: got %d, want 60", got)
	}
}
...
func testSum(t *testing.T) {
	...
	for _, c := range cases {
		if got := Sum(c.in); got != c.want {
			t.Fatalf("Sum(%v): got %d, want %d", c.in, got, c.want)
		}
	}
}
...
func testEvens(t *testing.T) {
	...
	for _, c := range cases {
		if got := Evens(c.in); !slices.Equal(got, c.want) {
			t.Fatalf("Evens(%v): got %v, want %v", c.in, got, c.want)
		}
	}
}
...
func testIndexOf(t *testing.T) {
	...
	for _, c := range cases {
		if got := IndexOf(c.xs, c.target); got != c.want {
			t.Fatalf("IndexOf(%v, %d): got %d, want %d", c.xs, c.target, got, c.want)
		}
	}
}
...
func testWordCount(t *testing.T) {
	...
	if !maps.Equal(got, want) {
		t.Fatalf("WordCount: got %v, want %v", got, want)
	}
	if got := WordCount(nil); len(got) != 0 {
		t.Fatalf("WordCount(nil): got %v, want empty", got)
	}
}
...
func testLookup(t *testing.T) {
	...
	if v, ok := Lookup(m, "hello"); v != 2 || !ok {
		t.Fatalf("Lookup(m, \"hello\"): got (%d, %v), want (2, true)", v, ok)
	}
	if v, ok := Lookup(m, "nope"); v != 0 || ok {
		t.Fatalf("Lookup(m, \"nope\"): got (%d, %v), want (0, false)", v, ok)
	}
}
