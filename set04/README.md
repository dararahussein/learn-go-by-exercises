...
1. Run `go test -run TestSlugify ./set04/` and read the deliberate failure.
2. Fill the case table in `a_slug_test.go` from the prose contract. Include at
...
5. Run `go test -cover ./set04/`, `go vet ./set04/`, and `go doc strings.Builder`.
