...
func testReadLines(t *testing.T) {
	...
	if err != nil || !slices.Equal(got, want) {
		t.Fatalf("ReadLines: got (%v, %v), want (%v, nil)", got, err, want)
	}
}
...
func testWordFrequency(t *testing.T) {
	...
	if err != nil || !maps.Equal(got, want) {
		t.Fatalf("WordFrequency: got (%v, %v), want %v", got, err, want)
	}
}
...
func testCountLines(t *testing.T) {
	...
	if got, err := CountLines(path); err != nil || got != 3 {
		t.Fatalf("CountLines: got (%d, %v), want (3, nil)", got, err)
	}
}

func testReadLinesMissing(t *testing.T) {
	if _, err := ReadLines(filepath.Join(t.TempDir(), "missing")); err == nil {
		t.Fatal("ReadLines(missing) error: got nil, want non-nil")
	}
}
